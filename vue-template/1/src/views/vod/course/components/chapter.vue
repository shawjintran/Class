...
import vindex from '@/views/vod/course/components/Video/index.vue'

export default {
  // 注册组件
  components: { ChapterForm, vindex },
  data() {
    return {
      chapterList: [] // 章节嵌套列表
    }
  ...
  methods: {
    ...
    // 添加课时
    addVideo(chapterId) {
      this.$refs.vindex.open(chapterId)
    },
    // 编辑课时
    editVideo(chapterId, videoId) {
      this.$refs.vindex.open(chapterId, videoId)
    },
    // 删除课时
