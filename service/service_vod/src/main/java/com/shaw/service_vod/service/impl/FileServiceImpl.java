...
import com.shaw.service_util.normal.ConstantPropertiesUtil;
...
public class FileServiceImpl implements FileService {
	...
	public String upload(MultipartFile file) throws IOException {
		...
		try{
			...
			// 例如对象键为folder/picture.jpg，则表示将文件 picture.jpg 上传到 folder 路径下
			String key = UUID.randomUUID().toString().replaceAll("-","")+
					file.getOriginalFilename();
			String dateUrl = new DateTime().toString("yyyy/MM/dd");
			key = dateUrl+"/"+key;
			//上传的流如果能获取准确的流长度,则推荐一定填写content-length
